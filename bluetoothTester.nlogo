...
globals [ setup-yet? move-distance ready-to-draw? color-from-remote ]
...
to show-previous-values-of [avar]
 clear-output
 let ls bluetooth:get-previous-values
 foreach ls
  [
   v ->
   if item 0 v = avar
    [
       output-print item 1 v
    ]
  ]
end
...
to draw-shape
  set move-distance 7
  let counter 0
  let sds bluetooth:get "NUM"
  let degree (360 / (sds))
  while [counter < sds]
  [
    create-turtles 1 [set heading degree * counter set color color-from-remote]
  ...
  ask turtles[
    change-heading sds
    pen-down
    fd move-distance + (move-distance * 2 / sds) ;;+ (move-distance - (sds + .5))
    pen-up
  ]
end

to change-heading[sides-num]
  let counter 0
  let start 15
  let degree 360 / sides-num
  set heading (heading + degree + (degree * (.25 * (sides-num - 2))))
end

to draw-continuous
  let prev-val bluetooth:get "IR"
  wait .05
  if bluetooth:get "IR" != prev-val
  [
     check-continuous
  ]
end
...
to check-continuous
  let val bluetooth:get "IR"
   if val = "blue"
   [
    ca
    set color-from-remote blue
    draw-shape
   ]
   if val = "red"
   [
    ca
    set color-from-remote red
    draw-shape
   ]
   if val = "green"
   [
    ca
    set color-from-remote green
    draw-shape
   ]
   if val = "yellow"
   [
    ca
    set color-from-remote yellow
    draw-shape
   ]
  if val = "select"
   [
